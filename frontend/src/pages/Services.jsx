  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      if (searchQuery && searchQuery.trim()) {
        // Use search endpoint
        url += `/search?search=${encodeURIComponent(searchQuery)}&page=${currentPage}&limit=${itemsPerPage}`
      } else if (selectedCategory !== 'all') {
        // Use search endpoint with category filter
        url += `/search?search=${selectedCategory}&page=${currentPage}&limit=${itemsPerPage}`
      } else {
      ...
      if (response.data.success) {
        ...
        setTotalCount(response.data.pagination?.totalServices || 0)
      }
    } catch (err) {
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-2 flex items-center gap-2">
                ...
                Browse and book healthcare services at your doorstep
              </p>
            </div>
              ...
              <div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-300 shadow-sm">
                ...
                <p className="text-gray-600 max-w-md mx-auto">
                  {searchQuery 
                    ...
                    ? `No services available in ${getCategoryLabel(selectedCategory)}`
                    ...
                    >
                      ...
                      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-primary-50 to-blue-50">
                        ...
                        <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-lg">
                          <span className="text-sm font-bold text-primary-600">{formatPrice(service.price)}</span>
                        </div>
                      ...
                      <div className="p-5">
                        ...
                        <p className="text-sm text-gray-700 line-clamp-2 mb-4 min-h-10">
                          {service.description || 'Professional healthcare service at your doorstep'}
                        </p>
                        ...
                        <div className="space-y-2 mb-4">
                          <div className="flex items-center text-sm text-gray-600">
                            ...
                            <span>{service.sessionDuration || 60} minutes session</span>
                          </div>
                          <div className="flex items-center text-sm text-gray-600">
                            <svg className="w-4 h-4 text-primary-500 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                            </svg>
                            <span>{service.averageRating?.toFixed(1) || '5.0'} rating</span>
                          </div>
                          <div className="flex items-center text-sm text-gray-600">
                            <svg className="w-4 h-4 text-primary-500 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
